...
export default function AddMenuItem({ onClose, onSuccess, onError }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Get current max ID to determine the next one
      // (needed because Supabase id column may not have auto-increment sequence)
      ...
      const payload = {
        ...
        sectionSub: formData.sectionSub || null,
        type: formData.type,
      ...
      const createdItem = (Array.isArray(data) && data.length > 0) ? data[0] : { ...payload };
      onSuccess(createdItem);
    } catch (error) {
      console.error('Error adding item:', error);
      onError(`Error adding item: ${error.message}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 16 }}>
      <div style={{ background: '#fff', padding: '28px 32px', borderRadius: 24, width: '100%', maxWidth: 460, boxShadow: '0 20px 40px rgba(0,0,0,0.1)', fontFamily: "'DM Sans', sans-serif", border: '1px solid rgba(180,83,9,0.1)' }}>
        <h3 style={{ marginTop: 0, marginBottom: 24, fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: 28, fontWeight: 700, color: '#1f2937' }}>Add New Item</h3>
        <form onSubmit={handleSubmit}>
          <div style={{ marginBottom: 16 }}>
            ...
            <input required type="text" value={formData.n} onChange={e => setFormData({ ...formData, n: e.target.value })} placeholder="e.g. Paneer Butter Masala" style={{ width: '100%', padding: '12px 14px', border: '1.5px solid #e5e7eb', borderRadius: 10, fontSize: 15, fontFamily: "'DM Sans', sans-serif", outline: 'none', transition: 'border-color 0.2s' }} onFocus={e => e.target.style.borderColor = '#d97706'} onBlur={e => e.target.style.borderColor = '#e5e7eb'} />
          </div>
          <div style={{ marginBottom: 16 }}>
            <label style={{ display: 'block', marginBottom: 6, fontSize: 13, fontWeight: 700, color: '#4b5563', letterSpacing: '0.05em' }}>Price</label>
            <input required type="number" value={formData.p} onChange={e => setFormData({ ...formData, p: e.target.value })} placeholder="250" style={{ width: '100%', padding: '12px 14px', border: '1.5px solid #e5e7eb', borderRadius: 10, fontSize: 15, fontFamily: "'DM Sans', sans-serif", outline: 'none', transition: 'border-color 0.2s' }} onFocus={e => e.target.style.borderColor = '#d97706'} onBlur={e => e.target.style.borderColor = '#e5e7eb'} />
          </div>
          <div style={{ marginBottom: 16 }}>
